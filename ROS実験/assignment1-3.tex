...
\begin{lstlisting}[caption=talker.pyのコード,label=lst:talker.py]
    #!/usr/bin/env python
...
\begin{lstlisting}[caption=listener.pyのコード,label=lst:listener.py]
    #!/usr/bin/env python
    import rospy
    from std_msgs.msg import String
...
% 以下に本文を書く
talker.pyを改変したコードtalker2.pyを以下のリスト\ref{lst:talker3.py}に示す．

\begin{lstlisting}[caption=talker2.pyのコード,label=lst:talker2.py]
    #!/usr/bin/env python
    # license removed for brevity
    import rospy
    from std_msgs.msg import String
    ...
    def talker():
        ...
        while not rospy.is_shutdown():
            message = "%s %s"%(raw_input(), rospy.get_time())
...
%以下に本文をかく
talker2.pyでは11行目で，表示するメッセージを作成している．raw_input()関数によってユーザによるキーボードからの入力を受け付けている．これとrospy.get_time()関数によって時刻を取得し，これをString型のmessage変数に代入している．キーボードからの入力を受けた時のみメッセージが送信されるようにするために，talker.pyの9行目と14行目にあるループに関するコードを削除している．
...
リスト\ref{lst:talker-listener.py}に，双方向でテキストを送受信可能なサービスとクライアントのコードtalker-listener.pyを示す。listener.pyのループに入っているコードを，talker2.pyのwhileループの中に取り込んでいる．またlistener.pyに含まれたいた，応答側のメッセージを生成するcallback関数も実装し，これをtalker_listener関数の中で呼び出している．

\begin{lstlisting}[caption=talker-listener.pyのコード,label=lst:talker-listener.py]
    #!/usr/bin/env python
    # license removed for brevity
    import rospy
    from std_msgs.msg import String
...
発展課題として教員のPCで起動したノードに対してトピック通信をおこなった．その結果を図\ref{fig:1-5-3}に示す．"hello"と送信した結果，talker-listenerによる送受信の結果が表示され，その下に教員のPCから自動返信された"とどきました!"の表示がされていることがわかる．また図\ref{fig:1-5_rqt}にこの時のrqtグラフを示す．ノード間の矢印が双方向に繋がっている他，教員のPCと自らのPCの他にも，他の受講生が繋いでいるPCも確認される．

\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{images/1-5-3.png}
    \caption{トピック通信をおこなった結果}
    \label{fig:1-5-3}
\end{figure}
\begin{figure}
    \centering
    \includegraphics[width=o.75\linewidth]{images/1-5_rqt.png}
    \caption{教員のPCと接続した時のrqtグラフ}
    \label{fig:1-5_rqt}
\end{figure}
